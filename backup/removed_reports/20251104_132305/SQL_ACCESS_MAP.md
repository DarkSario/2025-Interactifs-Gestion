# SQL Access Map
Generated: 2025-11-04T11:22:43.061283
...
- fetch patterns: 309
- row.get() usage: 68
- Positional indexing: 224
- execute() calls: 712
- sqlite3.connect() calls: 66
...
- `modules/buvette_db.py:31`
- `modules/db_api.py:15`
- `modules/buvette_mouvements_db.py:17`
- `modules/buvette_bilan_db.py:17`
...
### modules/buvette_bilan_db.py
- Line 49 `.fetchone()`: `""", (event_id, typ)).fetchone()`
- Line 87 `.fetchone()`: `row = conn.execute(q, params).fetchone()`
- Line 108 `.fetchone()`: `""", (event_id,)).fetchone()`
- Line 30 `.fetchall()`: `rows = conn.execute("SELECT id, name, date FROM events ORDER BY date DESC").fetchall()`
- Line 68 `.fetchall()`: `""", (inv_id,)).fetchall()`
...
### modules/buvette_mouvements_db.py
- Line 75 `.fetchone()`: `""", (mvt_id,)).fetchone()`
- Line 49 `.fetchall()`: `""").fetchall()`
- Line 126 `.fetchall()`: `rows = conn.execute("SELECT id, name FROM buvette_articles ORDER BY name").fetchall()`
- Line 137 `.fetchall()`: `rows = conn.execute("SELECT id, name FROM events ORDER BY date DESC").fetchall()`
...
- Line 76 `.fetchall()`: `return {row[0] for row in cursor.fetchall()}`
- Line 156 `.fetchall()`: `applied_details = {row[0]: row[1] for row in cursor.fetchall()}`
...
- Line 54 `.fetchall()`: `tables = [row[0] for row in cursor.fetchall()]`
- Line 60 `.fetchall()`: `columns = {row[1] for row in cursor.fetchall()}  # row[1] is column name`
...
- Line 64 `.fetchall()`: `return {row[1]: row[2] for row in cursor.fetchall()}`
...
### modules/buvette.py
- Line 460 (var: `item`): `unite_display = item.get("unite_type", item.get("unite", ""))`
- Line 460 (var: `item`): `unite_display = item.get("unite_type", item.get("unite", ""))`
- Line 461 (var: `item`): `quantite_display = item.get("quantite", "")`
- Line 465 (var: `item`): `iid=item.get("id", 0),`
- Line 467 (var: `item`): `item.get("name", ""),`
- Line 468 (var: `item`): `item.get("categorie", ""),`
- Line 469 (var: `item`): `item.get("stock", 0),`
- Line 472 (var: `item`): `item.get("contenance", ""),`
- Line 473 (var: `item`): `item.get("commentaire", "")`

### modules/buvette_bilan_db.py
- Line 89 (var: `row_dict`): `if row_dict and row_dict.get("qte"):`
- Line 110 (var: `row_dict`): `return row_dict.get("recette", 0.0) if row_dict else 0.0`
...
### scripts/apply_migrations.py
- Line 76: `return {row[0] for row in cursor.fetchall()}`
- Line 156: `applied_details = {row[0]: row[1] for row in cursor.fetchall()}`
...
### scripts/find_missing_columns.py
- Line 54: `tables = [row[0] for row in cursor.fetchall()]`
- Line 60: `columns = {row[1] for row in cursor.fetchall()}  # row[1] is column name`
...
### scripts/safe_add_columns.py
- Line 64: `return {row[1]: row[2] for row in cursor.fetchall()}`
